...
#23/07/21
#13/03/23 
#03/10/23 normalization of df with 3bs
#08/07/24 add lspc (plot class) 
###################
####IMPORTANT######
...
def csv_percentage_sorter(file, bp_number):
    ...
    #transform the values to percentage
    df = df.set_index('Div').div(bp_number).multiply(100)

    ##DESCOMENTAR PARA NORMALIZAR PARA 3Bs...en cada intervalo restar abundancia B- dividir por 3 y sumar nuevamente la abundancia
    #leer df 3B+
    #df = pd.read_csv('./DB8206_all_99999999.csv', sep = ' ')
    #df.set_index('Div', inplace=True)
    #leer df B- 
    #df2 = pd.read_csv('./LGE562_sampled_9100000.csv', sep = ' ')
    #df2.set_index('Div', inplace=True)
    #substract df-df2
    #dfsub = df.subtract(df2)
    #divide per 3
    #dfdiv = dfsub.div(3)
    #add df2
    #df_normal = dfdiv.add(df2).round(2)
    #para exportar csv descomentar
    ##df_normal.to_csv('./DB8206_all_99999999_normalizado.csv', sep = ' ')
    #df = pd.read_csv('./DB8206_all_99999999_normalizado.csv', delim_whitespace=True)
    #print('df index was renamed as "Div"')
    #df = df.rename(columns = {df.columns[0]:'Div'})
    #transform the values to percentage
    #df = df.set_index('Div').div(bp_number).multiply(100)
...
def landscapes_species_plotter(df, lolim_sat, uplim_sat, plot_name):
    ...
    plot.legend(loc=[1.01,0.01],ncol=2, prop={'size': 20})
...
def abundance_subtracter(df1, df2, plot_name):
    ...
    plot.legend(loc=[1.01,0.01],ncol=1,prop={'size': 20})
    #move x labels to the top
    plot.xaxis.tick_top()
...
if __name__ == "__main__":
    ...
    ###LSLP
	 ...
	 #checks if file has extension or not
        if plot_name == None: #no name
            ...
            plot_name = plot_name.split('.')[0] + '_sat_list.' + plot_name.split('.')[1]
	...
	##########se busca la interseccion de columnas df1 df2
        #########CONVERTIR ESTO EN FUNCION Y AGREGAR MENSAJE DE QUE SE ELIMINAN ELEMENTOS
...
################################################################
##########################################
########if df1.columns.difference(df2.columns):
########
        ...
        # Split the column name by '/'
            ...
            if len(parts) > 1:
                ...
                if prefix not in column_mapping:
                    column_mapping[prefix] = []
                column_mapping[prefix].append(col)
            else:
                # Handle columns without '/' (they remain as unique columns)
                column_mapping[col] = [col]

        # Merge columns based on prefixes and sum values across rows
        merged_data = {}
        for prefix, columns in column_mapping.items():
            if len(columns) == 1:
                merged_data[prefix] = df[columns[0]]
            else:
                merged_data[prefix] = df[columns].sum(axis=1)
        # Create the merged DataFrame
        merged_df = pd.DataFrame(merged_data)
        if plot_name == None: #no name
            plot_name = file + '_spc_landscape.png'
            csv_name = file + '_spc_landscape.csv'
        elif len(plot_name.split('.')) == 1: #no extension
            plot_name = plot_name + '_spc_landscape.png'
            csv_name = plot_name + '_spc_landscape.csv'
        else:
            plot_name = plot_name.split('.')[0] + '_spc_landscape.' + plot_name.split('.')[1]
            csv_name = plot_name.split('.')[0] + '.csv'

        landscapes_species_plotter(merged_df,lolim_sat, uplim_sat, plot_name)
        if csv_export:
            df_merged.to_csv(csv_name)
            print('The df was stored with the name {}'.format(csv_name))
        print('The plot was named as {}'.format(plot_name))
